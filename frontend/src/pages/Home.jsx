...
import { formatProduct, formatService } from '../lib/formatters';
...
function Home() {
  const productsQuery = useQuery({
    ...
    queryFn: () => api.products.list({ limit: 4 }),
  });
  ...
  const products = (productsQuery.data?.data?.products || []).map(formatProduct);
  const services = (servicesQuery.data?.data?.services || []).map(formatService);
