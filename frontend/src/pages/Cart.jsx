...
function Cart() {
  const { currentUser, cart, removeCartItem, updateCartItem } = useAppContext();
  ...
  const handleCardCheckout = async () => {
    ...
    try {
      ...
      const razorpay = new Razorpay({
        ...
        name: 'CampusConnect',
        description: `Purchase for ${cartItems.length} item${cartItems.length === 1 ? '' : 's'}`,
        prefill: {
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            {cartItems.map((item) => (
              ...
              >
                <img src={item.imageUrl} alt={item.title} className="h-28 w-full rounded-2xl object-cover sm:w-36" />
            ...
            <div className="mt-6 space-y-4 text-sm text-slate-600">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Card and UPI support coupon codes. Both provide secure, instant confirmation of your campus purchase.
                </p>
              </div>
