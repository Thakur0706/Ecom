  ...
  return (
    <article className="overflow-hidden rounded-2xl bg-white shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-lg">
      <div className="relative">
        ...
        />
        <span
          className={`absolute left-4 top-4 rounded-full px-3 py-1 text-xs font-semibold ${categoryStyles[product.category] || 'bg-slate-100 text-slate-700'}`}
        >
          {product.category}
        </span>
      </div>
      <div className="space-y-4 px-6 py-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`shrink-0 rounded-full px-3 py-1 text-xs font-semibold ${conditionStyles[product.condition] || 'bg-slate-100 text-slate-700'}`}
          >
            {product.condition}
          </span>
        </div>
